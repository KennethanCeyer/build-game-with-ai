...
class RuntimeSimulator:
    ...
    def _apply_location_effects(self, actor: Actor) -> None:
        ...
        if zone.zone_id == "maze_start":
            ...
            return
        if zone.zone_id == "maze_exit" and not self.state.flags.get("maze_escaped", False):
            if not self.state.flags.get("maze_started", False):
                self._event(
                    "미로 출구에 먼저 도착했습니다. 미로 검증은 시작 지점부터 통과해야 합니다.",
                    "warning",
                )
                return
            self.state.flags["maze_escaped"] = True
            self._event(f"{actor.display_name}가 미로 출구에 도착했습니다.")
