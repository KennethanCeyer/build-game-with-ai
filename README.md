...
| 분류 (담당) | 도구 명칭 | 기능 설명 |
| :--- | :--- | :--- |
| **지휘 (감독자)** | `exit_loop` | 목표 달성 또는 진행 불가능 시 시스템 전체 루프 안전 종료 |
| **전략 (전략가)** | `save_memory` | 발견한 경로, 전략 수정 사항 및 환경적 특이 사항을 작업 기억에 기록 |
| **관측 (관측자)** | `inspect_game_state` | 실시간 스크린샷 캡처, 캐릭터 좌표 및 주변 장애물 정보 수집 |
| | `adjust_camera_view` | 카메라 각도와 줌 변경을 통해 사각지대 탐색 및 추가 정보 확보 |
| **실행 (행동가)** | `apply_input_buffer` | 설계된 입력 시퀀스에 따라 실제 키보드 제어 명령 전송 |
