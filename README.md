...
| 에이전트 | 역할 | 모델 | 담당 업무 |
| :--- | :--- | :--- | :--- |
| **관측자** | 데이터 수집 | Gemini 3.1 Flash | 화면을 캡처하고 거리나 장애물 정보를 데이터로 정리해 보고합니다. |
| **전략가** | 작전 설계 | Gemini 3.1 Pro | 관측 데이터를 바탕으로 최적의 이동 경로와 물리 상수를 계산합니다. |
| **행동가** | 액션 실행 | Gemini 3.1 Flash | 설계된 전략에 맞춰 키를 누르고 물리적인 충돌에 대응합니다. |
| **감독자** | 지휘 및 검증 | Gemini 3.1 Pro/Flash | 전체 흐름을 지휘하며, 정체된 상황이 생기면 전략을 수정하도록 지시합니다. |
